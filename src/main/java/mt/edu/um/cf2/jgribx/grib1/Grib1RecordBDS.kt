...
class Grib1RecordBDS(
		...
		internal fun readFromStream(gribInputStream: GribInputStream,
			...
			if (bms != null) {
				...
				if (!isConstant) {
					...
					data = FloatArray(gds.cols * gds.rows)
			...
			if (!isConstant && data.size != gds.cols * gds.rows) {
				Logger.error("Grid should contain" +
						" ${gds.cols} * ${gds.rows} = ${gds.cols * gds.rows} values." +
	...
	override fun writeTo(outputStream: GribOutputStream) {
		Logger.debug("Writing GRIB1 Binary Data Section (BDS) - ${length} bytes")

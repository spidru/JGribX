...
class Grib1RecordBMS(private val numberOfUnusedBitsAtEndOfSection3: Int,
	...
	init {
		if (numberOfUnusedBitsAtEndOfSection3 >= 8) throw NoValidGribException(
				"numberOfUnusedBitsAtEndOfSection3 = ${numberOfUnusedBitsAtEndOfSection3} should be between 0 and 7")
	}

	override fun writeTo(outputStream: GribOutputStream) {
		Logger.debug("Writing GRIB1 Bit Map Section (BMS) - ${length} bytes")

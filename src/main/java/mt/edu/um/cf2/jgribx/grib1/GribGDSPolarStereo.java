...
public class GribGDSPolarStereo extends Grib1RecordGDS
{
    ...
    */
   public GribGDSPolarStereo(GribInputStream in, byte[] header)
          throws IOException, NoValidGribException, NotSupportedException
   {
      super(header);
      Logger.println("Discovered GDS type: PolarStereo", Logger.DEBUG);
      int[] data;
   ...
   public double[] getGridCoords()
   {
      ...
      for (int j = 0; j < grid_ny; j++)
      {
         ...
         for (int i = 0; i < grid_nx; i++)
         {
            ...
            {
               Logger.println("GribGDSPolarStereo: latitude out of range (-90 to 90).", Logger.ERROR);
            }
            coords[count++] = lon;
            coords[count++] = lat;
         }

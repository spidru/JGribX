...
import mt.edu.um.cf2.jgribx.grib1.Grib1RecordGDS;
...
public class GribGDSLambert extends Grib1RecordGDS
{
    ...
    */
   public GribGDSLambert(GribInputStream in, byte[] header)
      ...
      for (int j = 0; j < grid_ny; j++)
      {
         ...
         for (int i = 0; i < grid_nx; i++)
         {
            ...
            if (lat > 90.0 || lat < -90.0)
               Logger.println("GribGDSLambert: latitude out of range (-90 to 90).", Logger.ERROR);
            //coords[grid_nx * y + x] =
            coords[k++] = lon;
            coords[k++] = lat;
         }

 ...
 *
 * @See the GribPDSParameter, GribPDSParamTable, and GribPDSLevel classes. Added
 * another time field to differentiate between base time and forecast time.
 * Started handling Subcenters (though not completed yet) Added code to handle
 * time offsets (thanks Hans)
 */
...
public class GribRecordPDS {
     ...
     */
    private static Logger logger = Logger.getLogger(GribRecordPDS.class.getName());
...
// rdg - added the following:
    ...
    public GribRecordPDS(BitInputStream in) throws NotSupportedException, IOException {
        ...
        if (table_version == 255) {
            ...
            parameter = new GribPDSParameter(255, "missing", "missing parameter", "");
        } else {
...
//            parameter = parameter_table.getParameter(data[5]);
            parameter = GribPDSParamTable.getParameterFromFile(center_id, subcenter_id, table_version, data[5]);
        }
...
//                  "Range Indicator " + data[17] + " is not yet supported");
                System.err.println("GribRecordPDS: Time Range Indicator "
                        + data[17] + " is not yet supported - continuing, but time "
...
//      this.sub_center = data[22];
    ...
    public static String getOriginatingCentreName(int centre_id)
    {
        if (centre_id > 60)
        {
            System.err.println("Centre code name has not yet been added");
            return null;
        }
...
//       GribPDSLevel (with getPDSLevel) then calling its methods directly
    ...
    public Calendar getGMTBaseTime() {
        ...
        logger.debug("new time is " + gmtTime.getTime());
        return gmtTime;
    }
    ...
    public Calendar getGMTForecastTime() {
        ...
        logger.debug("forecast time = " + gmtTime.getTime());
        // hopefully this DST offset adjusts to DST automatically
        int dstOffset = gmtTime.get(Calendar.DST_OFFSET) / 3600000;
        int gmtOffset = gmtTime.get(Calendar.ZONE_OFFSET) / 3600000;//ms to hours
        logger.debug("offset is " + gmtOffset);
